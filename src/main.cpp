...
static const int64_t nTargetTimespan = 5 * 60;  // Kernalcoin - 5 mins
unsigned int nTargetSpacing = 60; // Kernalcoin - 60 seconds
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 6 * 60 * 60; // Kernalcoin - 6 hours
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60; // Kernalcoin - 30 days
unsigned int nModifierInterval = 10 * 60; // Kernalcoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "Kernalcoin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Kernalcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "KERNALCOIN 2015";
        CTransaction txNew;
        txNew.nTime = 1434742643;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1434742643;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2780593 : 2780593;
...
// <- GEN
        ...
        assert(block.hashMerkleRoot == uint256("0xa9f6417084ab20ff8bd05ed6c8c3769ffd3271fb0d30c0840b7a9859d4d5be97"));
